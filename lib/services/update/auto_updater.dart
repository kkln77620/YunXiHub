...
class AutoUpdater {
  ...
  Future<Map<String, dynamic>> _latestRelease() async {
    // YunXiHub 云服务器热更新接口（/api/stable?app=yunxihub）
    ...
    if (map['code'] != 0) {
      throw Exception('No update available: ${map['msg']}');
    }
    final versionName = map['version_name']?.toString() ?? '';
    final downloadUrl = map['download_url']?.toString() ?? '';
    if (versionName.isEmpty || downloadUrl.isEmpty) {
      throw Exception('Invalid update payload');
    }
    final isForced = (map['force'] as num?)?.toInt() == 1;
    // 转成与 GitHub release 兼容的结构，供 checkForUpdates 复用
    return {
      'tag_name': versionName,
      'body': map['desc']?.toString() ?? '',
      'html_url': downloadUrl,
      'published_at': '',
      'is_forced': isForced,
      'assets': [
        {
          'name': 'YunXiHub_android_$versionName.apk',
    ...
    KazumiDialog.show(
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            if (isAutoCheck && !updateInfo.isForced)
              TextButton(
                onPressed: () {
