...
import 'package:kazumi/services/remote/remote_resolver_client.dart';
...
class Plugin {
  ...
  Future<PluginSearchResponse> queryBangumi(
    ...
    // 远程解析优先（YunXiHub 服务器化解析），失败回退本地 RuleEngine
    if (RemoteResolverClient.instance.isEnabled) {
      try {
        final remote = await RemoteResolverClient.instance.search(
          toJson(),
          keyword,
        );
        if (remote.data.isNotEmpty) return remote;
  ...
  Future<List<Road>> queryChapterRoads(
    ...
    if (RemoteResolverClient.instance.isEnabled) {
      try {
        final remoteRoads = await RemoteResolverClient.instance.queryChapters(
          toJson(),
          source,
        );
        if (remoteRoads.isNotEmpty) return remoteRoads;
      } on CaptchaRequiredException {
        rethrow;
